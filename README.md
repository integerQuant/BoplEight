...
- The plugin targets BepInEx 5.4.23.2 and BoplEight 1.0.6 protocol version 3.
...
Run `./build.ps1` from this directory. The script runs 48 protocol/model/layout tests, compiles `dist/BoplEight.dll`, and installs it as `BepInEx/plugins/BoplEight.dll` unless `-NoInstall` is supplied.
...
Build the self-contained Windows installer with `./installer/build-installer.ps1`. The release executable is written to `installer/dist/BoplEight-Setup-1.0.6.exe` and includes BepInEx 5.4.23.2 plus BoplEight.
